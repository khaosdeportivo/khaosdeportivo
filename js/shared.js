...
function isCouponApplicable(coupon, cartItems) {
  // PRIORIDAD: productIds > appliesTo. Nunca sumar ambas (evita doble conteo).
  if (!coupon) return { applicable: false, reason: 'Cupón inválido', eligibleSubtotal: 0 };
  if (!cartItems) cartItems = [];

  const hasProductIds = coupon.productIds && coupon.productIds.length > 0;
  ...
  if (hasProductIds) {
    for (const item of cartItems) {
      if (coupon.productIds.includes(item.id) || coupon.productIds.includes(Number(item.id))) {
  ...
  for (const item of cartItems) {
    ...
    const isEligible =
      ...
      (coupon.appliesTo === 'calzado' && ['guayo','sintetica','futsal','zapatillas'].includes(meta.family)) ||
